...
import { act, render, waitFor, within, prettyDOM } from "@testing-library/react";
...
const configureStore = () => redux.createStore(reducers);
...
const mockForbiddenError = (reject) => () =>
    reject(
        new Error({
...
const allCourses = [...fakeData.courseRunData, ...fakeData.entitlementData];
...
const waitForEqual = async (valFn, expected, key) =>
    waitFor(() => {
        expect(valFn(), `${key} is expected to equal ${expected}`).toEqual(expected);
    });
const waitForRequestStatus = (key, status) => waitForEqual(() => getState().requests[key].status, status, key);
...
const courseNames = ["course-name-1", "course-name-2", "course-name-3"];
...
describe("ESG app integration tests", () => {
    ...
    describe("course cards", () => {
        const courseNames = ["course-name-0", "course-name-1", "course-name-2"];
